...
Host tools + browser activity panel. Package name, `cordis.patch.yml` mount `name`, and the client `__ModuleLoader__` id (read from `package.json` by `tsdown.config.ts`) must stay `@starpivot/dsh-agent-teams`.

This is not a live upstream fork. Lineage is pinned in `UPSTREAM.md`. Keep the original LICENSE.

## Differences from upstream

Read-only role isolation, barge-in delivery, no greeting turn, optional per-member worktrees, and plugin-defect reports to `Wuxie233/dsh-plugin-agent-teams`.
