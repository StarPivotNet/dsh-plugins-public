 ...
 */
export function memberPersona(team, member, stateDir, readOnlyRoles = []) {
 ...
 */
export function memberDispatchPrompt(team, task, brief) {
 ...
 */
export async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, stateDir, signal, firstTask, brief, worktree) {
    ...
    const startSpec = {
        ...
        request: {
            ...
            parent: captain,
            persona: memberPersona(team, member, worktree !== undefined ? join(captainWorkspace, stateDir) : stateDir, config.readOnlyRoles ?? []),
    ...
    if (worktree !== undefined) {
        ...
        if (headerCwd !== undefined && headerCwd !== worktree) {
            ...
            throw new Error(`agent-teams: the harness runtime did not apply the member worktree cwd (header says "${headerCwd}") — `
