...
- `/reboot` — restart the Host process on the same desktop port. The card says `正在重启，页面即将刷新`, then the page reloads after the new Host reconnects. A 15s cooldown blocks a second `/reboot`.
