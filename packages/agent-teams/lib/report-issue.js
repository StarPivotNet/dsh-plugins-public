...
export function runCommand(args) {
    ...
    if (command === undefined)
        return Promise.resolve({ exitCode: 1, stdout: '', stderr: 'No command provided' });
...
async function ensureLabel(repo, name, color, description, run) {
    await run([
 ...
 */
export function buildIssueBody(args, severity, reportedBy) {
    const lines = [
    ...
    if (args.trigger?.trim())
        lines.push('', '## Where this surfaced', args.trigger.trim());
    if (args.repro?.trim())
        lines.push('', '## Reproduction', args.repro.trim());
    if (args.proposal?.trim())
        lines.push('', '## Suggested direction', args.proposal.trim());
    lines.push('', '---', `Reported by \`agent_teams_report_issue\` from ${reportedBy}. Not yet triaged — confirm the defect against source before acting on it.`);
    return lines.join('\n');
}
/**
 ...
 */
export function reportIssueReporter(team, callerId) {
    if (team !== undefined && team.captainSessionId !== callerId) {
        throw new Error('Only the team captain can file plugin feedback. Send the finding to the captain with agent_teams_send_message instead.');
    }
 ...
 */
export async function executeTeamReportIssue(args, reportedBy, run = runCommand) {
    const title = args.title.trim();
    if (title === '')
        throw new Error('An issue title is required.');
    if (args.body.trim() === '')
        throw new Error('An issue body is required.');
