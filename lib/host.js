...
async function writeReloadProgress(settings, ns, progress) {
...
async function handleReload(ctx, settingsNs, rawInput, _agent) {
  ...
  const summary = failures.length === 0 ? `\u91CD\u8F7D\u5B8C\u6210\u3002\u5DF2\u91CD\u8F7D ${String(ok)} \u4E2A\u63D2\u4EF6\u3002${client}` : `\u91CD\u8F7D\u5B8C\u6210\uFF0C${String(ok)} \u4E2A\u6210\u529F\uFF0C\u5931\u8D25\uFF1A${failures.join("\uFF1B")}\u3002${client}`;
  await writeReloadProgress(settings, settingsNs, {
    phase: "done",
    current: "",
