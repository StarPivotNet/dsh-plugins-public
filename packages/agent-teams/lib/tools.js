...
import { appendMailbox, archiveTeamDir, CAPTAIN_KEY, createMessage, createTeamDir, findTeamByCaptain, findTeamByParticipant, readMailbox, readTeam, resolveTeamWorkspace, sanitizeKey, transitionError, unsatisfiedDependencies, withTeamLock, writeTeam, } from "./state.js";
import { assignedOpenTasks, assignedWorkBlock, deliverToMember, installMemberSelectionRuntime, interruptMember, memberActivity, resolveMemberLlmSelection, retireMember, spawnMember, } from "./members.js";
import { executeTeamReportIssue, reportIssueReporter, } from "./report-issue.js";
import { lastMatchingStallNotice, lastTurnEndKind, shouldNotifyMemberStall, stallCaptainMessage, } from "./stall.js";
...
function workspaceOf(agent, stateDir) {
 ...
 */
export function deliverCaptainReport(captain, from, content, mode = 'barge') {
    try {
        if (mode === 'barge')
 ...
 */
export function bargeCaptainReport(captain, from, content) {
...
export function resolveMemberSpawnBrief(args) {
    ...
    if (brief === '')
        throw new Error('spawn brief must not be empty — pass prompt, brief, instructions, task_description, or task_subject');
    return brief;
}
/** Parse the live-delivery mode. Default is barge so new instructions start now. */
export function parseDeliveryMode(value) {
    if (value === undefined || value.trim() === '' || value === 'barge')
        return 'barge';
    if (value === 'queue')
        return 'queue';
...
function memberWakeText(stateRoot, team, recipient, senderText) {
 ...
 */
function installMemberStallWatcher(ctx, config) {
 ...
 */
async function notifyCaptainOfMemberStall(ctx, config, agent) {
    try {
        ...
        if (captain !== undefined)
            deliverCaptainReport(captain, member.name, content, 'queue');
    }
    catch (error) {
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            return withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (subject === '')
                    throw new Error('task_subject must not be empty');
                ...
                await spawnMember(ctx, memberRuntime(config), memberSelections, selection, captain, fresh, member, config.stateDir, exec.signal, firstTask, brief, args.worktree, args.cwd);
            ...
            return withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (args.assignee !== undefined) {
                    try {
                        ...
                        throw new Error(`no active member named "${args.assignee}" in team "${fresh.name}" — spawn them with agent_teams_add_member first`);
                    }
                }
                const task = {
                    id: `t${fresh.taskSeq + 1}`,
