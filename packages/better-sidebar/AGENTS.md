...
StarPivot fork of [omdsh-dev/DSH-better-sidebar](https://github.com/omdsh-dev/DSH-better-sidebar). Install with `dsh plugin --profile web add github:StarPivotNet/dsh-plugins-public#path:packages/better-sidebar`. Do not also install upstream `dsh-better-sidebar`.

After a user-visible change: test and build from the repository root, bump `version`, push `main`, publish `@starpivot/dsh-better-sidebar`, and update `StarPivotNet/dsh-plugin-catalog` in the same turn.
