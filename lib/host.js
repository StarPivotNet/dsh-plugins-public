...
async function handleReload(ctx, settingsNs, rawInput) {
...
async function runDeferredReload(ctx, settingsNs, entryIds) {
  const registry = ctx.registry;
  const wanted = new Set(entryIds);
  const selected = [...ctx.loader.entries()].filter((entry) => !entry.options.group && wanted.has(entry.id)).map((entry) => ({
    id: entry.id,
    moduleName: String(entry.options.name ?? ""),
    enabled: !entry.disabled,
    get fiber() {
