...
import { readTeamSync, resolveTeamWorkspace, writeCaptainPointer } from "./state.js";
import { isReadOnlyRole, READ_ONLY_DENY_TOOLS, readOnlyPersonaRule } from "./roles.js";
 ...
 */
function memberDenyTools(member, config) {
...
const OPEN_TASK_STATUSES = new Set(['claimed', 'in_progress']);
/**
 ...
 */
export function assignedOpenTasks(team, memberName) {
 ...
 */
export function assignedWorkBlock(team, memberName) {
 ...
 */
export function memberPersona(team, member, stateDir, readOnlyRoles = [], workspaceRoot) {
 ...
 * @param workspaceRoot - absolute working directory the member must stay in.
 */
export function memberDispatchPrompt(team, task, brief, workspaceRoot) {
    const description = task.description === undefined || task.description === ''
        ? ''
 ...
 */
export async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, stateDir, signal, firstTask, brief, worktree, cwd) {
    ...
    const startSpec = {
        ...
        request: {
            ...
            parent: captain,
            persona: memberPersona(team, member, childCwd !== undefined && childCwd !== captainWorkspace ? join(captainWorkspace, stateDir) : stateDir, config.readOnlyRoles ?? [], childCwd),
    ...
    const start = await selections.withPending(captain.id, label, llmSelection, () => (ctx.subagents.startContinuable(startSpec)));
    if (childCwd !== undefined) {
        ...
        if (headerCwd !== undefined && headerCwd !== childCwd) {
            ...
            throw new Error(`agent-teams: the harness runtime did not apply the member cwd (header says "${headerCwd}") — `
 ...
 */
export async function deliverToMember(ctx, captain, childId, text, signal, mode = 'barge') {
    if (mode === 'barge')
 ...
 * @param childId - the member's durable child session id.
 */
export function retireMember(ctx, childId) {
 ...
 */
export function turnActivityOf(ctx, childId) {
...
export async function memberActivity(ctx, captainSessionId, signal) {
    ...
    for (const entry of entries) {
        if (entry.kind === 'child')
            activity.set(entry.id, turnActivityOf(ctx, entry.id));
    }
    return activity;
}
