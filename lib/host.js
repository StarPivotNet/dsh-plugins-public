...
// src/host/reload.ts
var SKELETON_ENTRY_IDS = /* @__PURE__ */ new Set([
  "webserver",
  "connection",
  "client-hmr",
  "modules",
...
function isExtraEntry(moduleName, dependencies) {
...
function selectReloadEntries(entries, matched, dependencies) {
  if (matched.kind === "one") {
  ...
  const selected = entries.filter((entry) => entry.enabled && !isSkeletonEntry(entry.id, entry.moduleName) && isExtraEntry(entry.moduleName, dependencies));
  return { ok: true, selected, skipped: entries.filter((entry) => entry.enabled).length - selected.length };
}
async function reloadHostEntry(entry, registry) {
  if (!entry.enabled) return { ok: false, message: `\u6761\u76EE ${entry.id} \u5DF2\u505C\u7528` };
  try {
    const fiber = entry.fiber;
    if (fiber !== void 0) {
      const callback = fiber.runtime?.callback;
      if (registry !== void 0 && callback !== void 0) {
...
async function requestBrowserReload(settings, ns) {
...
async function handleReload(ctx, profile, settingsNs, rawInput) {
  ...
  const hostLine = ok === 0 ? "\u6CA1\u6709\u53EF\u70ED\u91CD\u8F7D\u7684\u989D\u5916 Host \u63D2\u4EF6\u3002" : `\u5DF2\u91CD\u8F7D ${String(ok)} \u4E2A Host \u63D2\u4EF6\u3002`;
