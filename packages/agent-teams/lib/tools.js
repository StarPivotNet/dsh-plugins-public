...
import { deliverToMember, installMemberSelectionRuntime, interruptMember, memberActivity, resolveMemberLlmSelection, retireMember, spawnMember, } from "./members.js";
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(args, exec) {
            ...
            return withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                if (brief === '')
                    throw new Error('prompt must not be empty — the first turn is the first assigned task');
                ...
                await spawnMember(ctx, memberRuntime(config), memberSelections, selection, captain, fresh, member, config.stateDir, exec.signal, firstTask, brief, args.worktree);
