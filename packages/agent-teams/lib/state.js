 ...
 */
export async function writeCaptainPointer(worktree, stateDir, pointer) {
 ...
 */
export function resolveTeamWorkspace(ownCwd, stateDir) {
    try {
        const parsed = JSON.parse(readFileSync(join(ownCwd, stateDir, CAPTAIN_POINTER_FILE), 'utf8'));
        if (typeof parsed === 'object' && parsed !== null) {
            const captain = parsed.captainWorkspace;
            if (typeof captain === 'string' && captain.startsWith('/'))
                return captain;
        }
    }
    catch {
